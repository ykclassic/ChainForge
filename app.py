...
# FIXED IMPORT: Points to the 'modules' folder
try:
...
def main():
    ...
    imbalance = (sum([x[1] for x in ob['bids']]) - sum([x[1] for x in ob['asks']])) / (sum([x[1] for x in ob['bids']]) + sum([x[1] for x in ob['asks']]))
